...
// Map-page fetch: every approved place with BOTH happy_hours and patios
// left-joined (unlike getPlacesWithPatios, which uses patios!inner and
// only returns places that HAVE a patio). This lets the map's Happy
// Hours / Patios / Both toggle filter a single unified list client-side
// instead of juggling two separate queries.
export async function getPlacesForMap() {
  if (!supabase) return null;
  const { data, error } = await supabase
    .from("places")
    .select(
      "id, name, slug, neighborhood, address, website_url, menu_url, reservation_url, description, image_url, " +
        "happy_hours ( days, start_time, end_time, deals ), " +
        "patios ( patio_type, covered, heated, fire_pit, misters, shade, dog_friendly, view, tvs, live_music, seats, reservable, season_start, season_end, days, open_time, close_time, best_time, vibe, features, verified_at )"
    )
    .eq("status", "approved")
    .order("neighborhood", { ascending: true })
    .order("name", { ascending: true });
  ...
  return (data || []).map((p) => ({
    ...p,
    patio: Array.isArray(p.patios) ? p.patios[0] : p.patios,
  }));
}
...
export async function getPlacesWithPatios() {
  if (!supabase) return null;
  const { data, error } = await supabase
    .from("places")
    .select(
      "id, name, slug, neighborhood, address, website_url, menu_url, reservation_url, description, image_url, " +
        "patios!inner ( patio_type, covered, heated, fire_pit, misters, shade, dog_friendly, view, tvs, live_music, seats, reservable, season_start, season_end, days, open_time, close_time, best_time, vibe, features, verified_at )"
    )
    .eq("status", "approved")
    .order("neighborhood", { ascending: true })
    .order("name", { ascending: true });
  ...
  // Flatten the embedded array (unique per place) to a single `patio` object so
  // the directory can read p.patio.* directly.
  return (data || []).map((p) => ({
    ...p,
    patio: Array.isArray(p.patios) ? p.patios[0] : p.patios,
  }));
...
// ---- Date Night (recognized restaurants: Michelin / Bib / 50 Best / NYT) ----
export async function getPlacesWithDateNights() {
  if (!supabase) return null;
  const { data, error } = await supabase
    .from("places")
    .select(
      "id, name, slug, neighborhood, address, website_url, menu_url, reservation_url, description, image_url, " +
        "date_nights!inner ( michelin_stars, bib_gourmand, fifty_best, nyt, cuisine, price, reservation_platform, reservation_lead, vibe, verified_at )"
    )
    .eq("status", "approved")
    .order("name", { ascending: true });
  if (error) {
  ...
  return (data || []).map((p) => ({
    ...p,
    dn: Array.isArray(p.date_nights) ? p.date_nights[0] : p.date_nights,
  }));
}
